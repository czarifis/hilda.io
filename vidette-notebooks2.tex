...
\projname\ notebooks provide constructs that facilitate the process of building interactive notebooks. \eat{\yannis{out: ``simplify the process described in the last section."} }Beginning with the features that pertain to data analysts, \projname\ notebooks support two types of modules that streamline data access and visualizations, namely: \textbf{source wrappers} and \textbf{visual units}. Since these modules come preinstalled with \projname\ notebooks, analysts do not need to install any third-party packages in order to retrieve or visualize data. \eat{\yannis{IMPORTANT: I wouldn't finish the list of contributions at this point because it sounds as if the contribution is libraries. Academia is almost never good at doing this and even UCSD is good at doing this, people will ask ``show me the system then"}}
