...
Interactive notebooks allow the use of popular, high-level and highly expressive imperative languages, such as python, for composing a wide range of data analytics projects. The provided interface, enables data scientists to perform a data analysis and compose their findings into easily readable report-like web pages, that can include re-runnable code, visualizations and textual description of the included steps, all in one place. Data scientists can then share such pages with other users in order to present their findings, collaborate and further explore the underlying data.
\eat{
...
To address these issues, we propose {\projname}, an interactive notebook engine that employs a declarative language to facilitate communication with database sources, data curation and creation of interactive visualizations that can react to the reader's input. \projname\ also supports a propagation algorithm capable of using the reader's input to automatically reflect changes to subsequent parts of the notebook, thus offering enhanced data exploratory capabilities to the readers of notebooks, without requiring any coding skills.
