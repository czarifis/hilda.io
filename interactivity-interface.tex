...
\noindent{\bf Diffs.} Since, as explained above, both \projname\ variables and template instance are represented using JSON, changes to either of them are represented as diffs to JSON values. A \emph{diff} to a JSON value is of the form $\diff{}{}{\pp{p}}$, where $\pp{p}$ is the path to the element that is being modified. The first token of that path $p$ denotes the variable that is being mutated, while the remaining set denotes the individual nested attribute or array element. Specifically, in order to describe an individual attribute $k$ of a JSON value identified by path $p$ we use the notation $p.k$ and in order to describe the $i$-th element of the array targeted by a path $p$ we use the notation $p[k]$.
